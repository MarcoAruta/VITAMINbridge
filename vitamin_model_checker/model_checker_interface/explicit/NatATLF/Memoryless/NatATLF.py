from vitamin_model_checker.model_checker_interface.explicit.NatATLF.Memoryless.strategies import initialize, \
    generate_strategies, generate_guarded_action_pairs
from vitamin_model_checker.model_checker_interface.explicit.NatATLF.Memoryless.pruning import pruning
...
def model_checking(formula, model):
    ...
    while not found_solution and i <= k:
        ...
        for current_strategy in strategies_generator:  # Iterate over the generator object
            found_solution, fks, res = pruning(cgs, model, agents, CTLformula, current_strategy)  # Call pruning here
            if found_solution:
        ...
        if (not found_solution):
            print(f"False, the initial state has never been completely satisfied!")
